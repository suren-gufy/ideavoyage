...
const analyzeStartupWithAI = async (idea) => {
...
Please provide detailed analysis in JSON format:
{
  ...
  "reasoning": "Why these subreddits and analysis are relevant"
      ...
      if (hasPerplexityKey) {
        ...
        if (response.ok) {
          ...
          console.log(`⚠️ OpenAI API error: ${response.status} ${response.statusText}`);
        }
      }
  ...
  const keywordMap = {
    ...
    // Creative specific words
    'umbrella': ['BuyItForLife', 'WeatherGifs', 'mildlyinteresting', 'gadgets'],
    'rain': ['weather', 'WeatherGifs', 'mildlyinteresting', 'gadgets'],
    'voting': ['politics', 'technology', 'cryptography', 'blockchain'],
    'election': ['politics', 'technology', 'democracy', 'voting'],
    'meal': ['Cooking', 'MealPrepSunday', 'nutrition', 'HealthyFood', 'food'],
    'planning': ['productivity', 'organization', 'MealPrepSunday', 'lifehacks'],
    'introverted': ['introvert', 'socialskills', 'anxiety', 'socialanxiety'],
    'friends': ['socialskills', 'relationships', 'MakeNewFriendsHere', 'introvert'],
    'sauce': ['Cooking', 'spicy', 'food', 'recipes', 'HotSauce'],
    'hot': ['spicy', 'Cooking', 'food', 'HotSauce'],
    'artisanal': ['crafts', 'food', 'smallbusiness', 'Entrepreneur'],
    'mirror': ['DIY', 'HomeImprovement', 'gadgets', 'selfimprovement'],
    'posture': ['posture', 'fitness', 'Health', 'bodyweightfitness'],
    'coffee': ['Coffee', 'espresso', 'barista', 'cafe'],
    'roast': ['Coffee', 'espresso', 'food', 'Cooking'],
    'crossword': ['crossword', 'puzzles', 'wordgames', 'dailypuzzles'],
    'puzzle': ['puzzles', 'crossword', 'riddles', 'brain_training'],
    'plant': ['houseplants', 'gardening', 'plants', 'IndoorGarden'],
    'soil': ['gardening', 'houseplants', 'plants', 'botany'],
    'monitoring': ['technology', 'gadgets', 'IoT', 'smarthome'],
    'sensor': ['technology', 'gadgets', 'IoT', 'smarthome', 'electronics'],
    'trading': ['TradingCards', 'collectibles', 'pokemon', 'gaming', 'investing'],
    'card': ['TradingCards', 'collectibles', 'pokemon', 'gaming', 'cards']
  };
  ...
  if (matchedSubreddits.length > 0) {
    ...
    const subredditCounts = matchedSubreddits.reduce((acc, sub) => {
      acc[sub] = (acc[sub] || 0) + 1;
      return acc;
    }, {});
...
module.exports = { analyzeStartupWithAI };